<template>
  ...
  import JobService from "@/services/DeviceService";
  ...
  export default defineComponent({
    ...
    data: () => ({
      job: {} as Job,
      jobStatuses: Object.values(JobStatusEnum),
      selectedStatus: '',
      
    }),
    methods: {
      async handleSubmit() {
        if (this.job.commands == null) {
          this.job.commands = [];
        }
        this.job.currentStatus = this.selectedStatus;
  
        //const response: ResponseData<Job> = await JobService.createDevice(this.device);
        // Clear the form
        this.job = {};
      },
    },
